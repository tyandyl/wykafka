...
/**
 * 每个KafkaProducer都有一个消息缓存类accumulator，这个缓存类有一个map存储TopicPartiton-->List<RecordBatch>键值对。
 * 用户每次发送消息都是先添加到该消息对应TopicPartition的RecordBatch中。而RecordBatch中有一个大小限制的ByteBuffer，
 * 存储用户发送的消息。
 */
public final class RecordAccumulator {
    ...
    //这其实是为了处理“发送失败，重试“的问题：当消息发送失败，要重发的时候，需要把消息优先放入队列头部重新发送，这就需要用到双端队列，
    // 在头部，而不是尾部加入。当然，即使如此，该消息发出去的顺序，还是和Producer放进去的顺序不一致了
    private final ConcurrentMap<TopicPartition, Deque<ProducerBatch>> batches;
    private final IncompleteBatches incomplete;//用于记录未完成的所有batch
    ...
    public RecordAppendResult append(TopicPartition tp,
        ...
        try {
            ...
            synchronized (dq) {
                ...
                //第一次，这里应该是空的，因为getOrCreateDeque赋值了一个空的arrayDeque
            ...
            //只有大小为batch.size的内存申请才会从free中获取，所以消息大小尽量不要大于batch.size，
            // 这样才能充分利用缓存池。
            // 为什么申请的内存会不等于batch.size呢，
            // 原因是在RecordAccumulator#append中有一句 int size = Math.max(this.batchSize, Records.LOG_OVERHEAD + Record.recordSize(key, value)),
            // 即如果消息大小大于batch.size则会使用消息的大小申请内存。
    ...
    private MemoryRecordsBuilder recordsBuilder(ByteBuffer buffer, byte maxUsableMagic) {
        ...
        //新建一个MemoryRecordsBuilder，注意0L是baseOffset
        return MemoryRecords.builder(buffer, maxUsableMagic, compression, TimestampType.CREATE_TIME, 0L);
    }

    //http://matt33.com/2018/10/24/kafka-idempotent/?hmsr=pingdoc.io&utm_medium=pingdoc.io&utm_source=pingdoc.io
    //http://mengyou0304.github.io/2015/10/11/kafka_producer_%E4%BB%A3%E7%A0%81%E9%98%85%E8%AF%BB/
    private RecordAppendResult tryAppend(long timestamp, byte[] key, byte[] value, Header[] headers,
                                         Callback callback, Deque<ProducerBatch> deque) {
        //检索此deque队列的最后一个元素(但不删除)。返回null如果此deque队列为空。
        ProducerBatch last = deque.peekLast();
        if (last != null) {
            //这个方法主要是看看recordbatch里面地方够不够，不够就返回null，够就加上，
            // Recordbatch里用一个List<Thunk>  来存储每个msg的callback
