...
public class DefaultRecordBatch extends AbstractRecordBatch implements MutableRecordBatch {
    static final int BASE_OFFSET_OFFSET = 0;//基准,表示当前RecordBatch的起始位移。
    static final int BASE_OFFSET_LENGTH = 8;//基础长度,计算partition leader epoch到headers之间的长度。
    static final int LENGTH_OFFSET = BASE_OFFSET_OFFSET + BASE_OFFSET_LENGTH;//这是length的起始位置
    static final int LENGTH_LENGTH = 4;//这是length的长度
    static final int PARTITION_LEADER_EPOCH_OFFSET = LENGTH_OFFSET + LENGTH_LENGTH;//partition leader的起始位置
    static final int PARTITION_LEADER_EPOCH_LENGTH = 4;//partition leader的长度
    //消息格式的版本号，对于v2版本而言，magic等于2
    static final int MAGIC_OFFSET = PARTITION_LEADER_EPOCH_OFFSET + PARTITION_LEADER_EPOCH_LENGTH;//magic的起始位置
    static final int MAGIC_LENGTH = 1;//magic的长度
    static final int CRC_OFFSET = MAGIC_OFFSET + MAGIC_LENGTH;//crc的起始位置
    static final int CRC_LENGTH = 4;//crc的长度
    //消息属性，注意这里占用了两个字节。低3位表示压缩格式，可以参考v0和v1；第4位表示时间戳类型；
    // 第5位表示此RecordBatch是否处于事务中，0表示非事务，1表示事务。第6位表示是否是Control消息，0表示非Control消息，而1表示是Control消息，Control消息用来支持事务功能。
    static final int ATTRIBUTES_OFFSET = CRC_OFFSET + CRC_LENGTH;//这个是attributes的其实位置
    static final int ATTRIBUTE_LENGTH = 2;//这个是attributes的长度
    //RecordBatch中最后一个Record的offset与first offset的差值。主要被broker用来确认RecordBatch中Records的组装正确性。
    public static final int LAST_OFFSET_DELTA_OFFSET = ATTRIBUTES_OFFSET + ATTRIBUTE_LENGTH;//这是 last_offset的起始位置
    static final int LAST_OFFSET_DELTA_LENGTH = 4;//这是 last_offset的长度
    ...
    //RecordBatch中最大的时间戳，一般情况下是指最后一个Record的时间戳，和last offset delta的作用一样，用来确保消息组装的正确性。
     ...
     * 消息的元数据的长度，参看https://www.cnblogs.com/qwangxiao/p/9043491.html
     */
